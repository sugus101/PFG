...
contract ApuestasDistribuidas {
    ...
    // -------------------------------
    enum EleccionApuesta {
        //Enum para la eleccion de la apuesta
        Nula,
        Si,
        No
    } // Nula = 0 ; Si = 1 ; No = 2

    struct ApuestaIndividual {
        //Struct para una apuesta individual
    ...
    struct EventoDeApuesta {
        //Struct para un evento / mercado (incluye las apuestas)
        string descripcion;
    ...
    address payable public propietario; //El dueno del contrato. Payable para que pueda recibir fondos
    ...
    // -------------------------------
    event EventoDeApuestaCreado(uint indexed betId, string descripcion, uint cuotaSi, uint cuotaNo, uint fechaLimite, address creador);
    event EventoApuestaRealizada(uint indexed betId, address indexed apostante, uint montante, EleccionApuesta eleccion);
    event EventoMercadoCerrado(uint indexed betId, uint cerradaEn);
    event EventoMercadoResuelto(uint indexed betId, EleccionApuesta resultadoGanador, uint montanteTotalPagado);
    ...
    event EventoDeApuestaEditado(uint indexed betId, string nuevaDescripcion, uint nuevaCuotaSi, uint nuevaCuotaNo);

    // -------------------------------
    // 0.2 - MODIFICADOR PARA EL OWNER
    // -------------------------------
    modifier soloPropietario() {
        //Esto es solo para limitar determinadas funciones al owner
        require(msg.sender == propietario, "Solo el propietario puede llamar esta funcion");
        _;
    }

    function getDirecPropietario() public view returns (address) {
        return propietario;
    }
    ...
    // 0.3 - CONSTRUCTOR
    // -------------------------------
    constructor(address payable creador) payable {
        //tengo que poner payable aqui tambien para que cuadre con la declaracion de la variable
        //Lo pongo payable para inciar el contrato con un balance inicial (lo indico en el deploy_my_contract)
        propietario = creador;
    ...
    // -------------------------------
    function crearEventoDeApuesta(string memory _descripcion, uint _cuotaSi, uint _cuotaNo, uint _duracionApuestasEnSegundos) public soloPropietario {
        //Nota gus: en string le pongo memeory al ser tipo complejo y no necesitarlo despues de ej la funcion
    ...
    // -------------------------------
    // 1.1bis CREAR evento S  DE APUESTA (BATCH 10 MERCADOS) - SOLO PROPIETARIO
    // -------------------------------
    function crearEventosDeApuesta(
        string[] memory _descripciones,
        uint[] memory _cuotasSi,
        uint[] memory _cuotasNo,
        uint[] memory _duracionesApuestasEnSegundos
    ) public soloPropietario {
        // Para recorrerlas
        for (uint i = 0; i < _descripciones.length; i++) {
            // Bloque de de requires
            require(
                _descripciones.length == _cuotasSi.length && _cuotasSi.length == _cuotasNo.length && _cuotasNo.length == _duracionesApuestasEnSegundos.length,
                "Todos los arrays de entrada deben tener la misma longitud."
            ); //para ver que no se descuadren las longitudes

            require(_cuotasSi[i] > 10000, "La cuota para -Si- debe ser mayor que 1x (10000%)");
            require(_cuotasNo[i] > 10000, "La cuota para -No- debe ser mayor que 1x (10000%)");
            require(_duracionesApuestasEnSegundos[i] > 0, "La duracion de las apuestas debe ser positiva");

            uint idActual = proximoBetId;

            EventoDeApuesta storage nuevaApuesta = detallesEventoDeApuesta[idActual];

            nuevaApuesta.descripcion = _descripciones[i];
            nuevaApuesta.cuotaSi = _cuotasSi[i];
            nuevaApuesta.cuotaNo = _cuotasNo[i];
            nuevaApuesta.fechaLimiteApuestas = block.timestamp + _duracionesApuestasEnSegundos[i];
            nuevaApuesta.estaAbierta = true;
            nuevaApuesta.estaResuelta = false;
            nuevaApuesta.resultadoGanador = EleccionApuesta.Nula;

            proximoBetId++;

            // Emite un evento para cada mercado creado
            emit EventoDeApuestaCreado(idActual, _descripciones[i], _cuotasSi[i], _cuotasNo[i], nuevaApuesta.fechaLimiteApuestas, msg.sender);
        }
    }
    ...
    function resolverEventoDeApuesta(uint _betId, EleccionApuesta _resultado) public soloPropietario {
        ...
        for (uint i = 0; i < eventoActual.apuestasActuales.length; i++) {
            //recorro las apuestas actuales
            if (eventoActual.apuestasActuales[i].eleccion == _resultado) {
                //Selecciono las apuestas que coinciden con el resultado
    ...
    // -------------------------------
    function obtenerEstadoEventoDeApuesta(
        uint _betId // Pongo view para no gastar GAS
    )
        public
        view
        returns (
            ...
            uint poolTotalSi,
            uint poolTotalNo,
            uint numeroDeApuestasIndividuales
        )
    {
        // BLOQUE DE REQUIRES PARA QUE NO RECIBA DATOS ERRONEOS
        ...
        return (
            ...
            apuesta.cuotaSi,
            apuesta.cuotaNo,
            apuesta.fechaLimiteApuestas,
            apuesta.estaAbierta,
            apuesta.estaResuelta,
            apuesta.resultadoGanador,
            apuesta.montanteTotalApostadoASi,
            apuesta.montanteTotalApostadoANo,
            apuesta.apuestasActuales.length
        );
    }
    ...
    // -------------------------------
    function obtenerDetallesApuestaIndividual(
        uint _betId,
        uint _indiceApuesta
    ) public view returns (address apostante, uint montante, EleccionApuesta eleccion, bool pagado) {
        // BLOQUE DE REQUIRES PARA QUE NO RECIBA DATOS ERRONEOS
    ...
    // -------------------------------
    // 5.1 RETIRAR FONDOS DEL CONTRATO - SOLO PROPIETARIO
    // -------------------------------
    function retirarFondosDelContrato(uint _cantidad) public soloPropietario {
        // BLOQUE DE REQUIRES PARA QUE NO RECIBA DATOS ERRONEOS
        require(_cantidad > 0, "La cantidad a retirar debe ser mayor que cero.");
        require(address(this).balance >= _cantidad, "Fondos insuficientes en el contrato.");

        // Transferir los fondos al propietario
        propietario.transfer(_cantidad);

        // Emitir un evento para registrar la retirada
        emit EventoFondosRetirados(propietario, _cantidad);
    }
}
